<?php

$input_name = 'files';
//$path = $_SERVER['DOCUMENT_ROOT'] . '/datasheets/';
$path = filter_input(INPUT_POST, 'DOCUMENT_ROOT').'/datasheets/';
$error = $success = '';
include_once 'class/class_op_dlg.php';
$p = new class_op_dlg();
$fname = $_FILES[$input_name];
$result = $p->compliteLoad($fname);
if ($result != '') {
    $response = $result;
} else {
            // Перемещаем файл в директорию /datasheets/.
        $newname = $p->translitera($fname['name']);
        if (move_uploaded_file($fname['tmp_name'], $path . $newname)) {
            // Далее можно сохранить название файла в БД и т.п.
            $response = $newname;
            $success = '<p style="color: green">Файл «' . $fname['name'] . '» успешно загружен.</p>';
        } 
    }
header('Content-Type: application/json');
echo json_encode($response, JSON_UNESCAPED_UNICODE);
exit();
